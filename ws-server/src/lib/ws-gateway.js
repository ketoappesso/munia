...
class FacegateGateway {
  ...
  async updateDeviceOnlineStatus(deviceId, online) {
    try {
      ...
      // prisma client is already imported at the top
      await prismaClient.facegateDevice.update({
        where: { deviceId },
        data: { online }
      });
    } catch (err) {
  ...
  async handleRegisterDevice(ws, params, req_id) {
    ...
    await prismaClient.facegateDevice.upsert({
  ...
  async handleUploadRecords(ws, params, req_id) {
    ...
    for (const record of Records) {
      try {
        await prismaClient.facegateRecord.upsert({
          where: {
            deviceId_recordId: {
              deviceId: DeviceId,
  ...
  async handleGetPersonCount(ws, params, req_id) {
    // prisma client is already imported at the top
    const count = await prismaClient.facegatePerson.count();
  ...
  async handleGetPersonsByPage(ws, params, req_id) {
    ...
    const persons = await prismaClient.facegatePerson.findMany({
      skip: Offset || 0,
  ...
  async handleGetPersonsById(ws, params, req_id) {
    ...
    // prisma client is already imported at the top
    const dbPersons = await prismaClient.facegatePerson.findMany({

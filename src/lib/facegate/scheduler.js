...
class FacegateScheduler {
    ...
    try {
      ...
      for (const schedule of schedules) {
        ...
        for (const target of schedule.targets) {
          ...
          if (!schedule.cron) {
            const existingJob = await prisma.facegateJob.findFirst({
              where: {
            ...
            const recentJob = await prisma.facegateJob.findFirst({
              where: {
                scheduleId: schedule.id,
                deviceId: target.deviceId,
                updatedAt: { gte: since }
          ...
          // Create new job
          await prisma.facegateJob.create({
            data: {
              scheduleId: schedule.id,
              deviceId: target.deviceId,
              state: 'pending'
  ...
  async dispatchPendingJobs() {
    try {
      const jobs = await prisma.facegateJob.findMany({
        where: { state: 'pending' },
        include: {
          schedule: {
            include: { image: true }
      ...
      for (const job of jobs) {
        ...
        try {
          ...
          // Mark job as sent
          await prisma.facegateJob.update({
            where: { id: job.id },
            data: { state: 'sent' }
          });
        } catch (err) {
  ...
  async requeueFailedJobs() {
    try {
      ...
      const failedJobs = await prisma.facegateJob.findMany({
        where: {
          state: 'failed',

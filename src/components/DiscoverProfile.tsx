...
export const DiscoverProfile = memo(
  ({ userId }: { userId: string }) => {
    ...
    if (isError) return <div>加载资料出错。</div>;
    if (!user) return null;
    ...
    return (
      <div className="gap-4 drop-shadow-sm">
        ...
        <div className="flex flex-col items-center rounded-b-3xl bg-card py-8">
          ...
          <p className="mb-4 px-2 text-center text-muted-foreground">{user.bio || '暂无简介'}</p>
          <div className="flex gap-6">
            <p className="flex justify-center gap-1 text-lg font-semibold">
              <span>{user.followerCount}</span> <span className="text-muted-foreground">粉丝</span>
            </p>
            <p className="flex justify-center gap-1 text-lg font-semibold">
              <span>{user.followingCount}</span> <span className="text-muted-foreground">关注</span>
            </p>
          </div>
        </div>
      </div>
